<?php

namespace App\DataTables;

use App\Models\receives;
use Form;
use Yajra\Datatables\Services\DataTable;

class receivesDataTable extends DataTable
{

    /**
     * @return \Illuminate\Http\JsonResponse
     */
    public function ajax()
    {
        return $this->datatables
            ->eloquent($this->query())
            ->addColumn('action', 'receives.datatables_actions')
            ->make(true);
    }

    /**
     * Get the query object to be processed by datatables.
     *
     * @return \Illuminate\Database\Query\Builder|\Illuminate\Database\Eloquent\Builder
     */
    public function query()
    {
        $receives = receives::query();

        return $this->applyScopes($receives);
    }

    /**
     * Optional method if you want to use html builder.
     *
     * @return \Yajra\Datatables\Html\Builder
     */
    public function html()
    {
        return $this->builder()
            ->columns($this->getColumns())
            ->addAction(['width' => '10%'])
            ->ajax('')
            ->parameters([
                'dom' => 'Bfrtip',
                'scrollX' => false,
                'buttons' => [
                    // 'print',
                    'reset',
                    'reload',
                    [
                         'extend'  => 'collection',
                         'text'    => '<i class="fa fa-download"></i> Export',
                         'buttons' => [
                             'csv',
                             'excel',
                             // 'pdf',
                         ],
                    ],
                    'colvis'
                ]
            ]);
    }

    /**
     * Get columns.
     *
     * @return array
     */
    private function getColumns()
    {
        return [
            'invoiceID' => ['name' => 'invoiceID', 'data' => 'invoiceID'],
            'customerID' => ['name' => 'customerID', 'data' => 'customerID'],
            'invoiceTotal' => ['name' => 'invoiceTotal', 'data' => 'invoiceTotal'],
            'receiveTotal' => ['name' => 'receiveTotal', 'data' => 'receiveTotal'],
            'refundTotal' => ['name' => 'refundTotal', 'data' => 'refundTotal'],
            'createdDate' => ['name' => 'createdDate', 'data' => 'createdDate'],
            'createdUserID' => ['name' => 'createdUserID', 'data' => 'createdUserID'],
            'modifiedDate' => ['name' => 'modifiedDate', 'data' => 'modifiedDate'],
            'modifiedUserID' => ['name' => 'modifiedUserID', 'data' => 'modifiedUserID']
        ];
    }

    /**
     * Get filename for export.
     *
     * @return string
     */
    protected function filename()
    {
        return 'receives';
    }
}
